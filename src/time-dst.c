...
static inline int decode(const void *ptr) {
	return (int)be32toh(*(int *)ptr);
}

static inline int64_t decode64(const void *ptr) {
	return (int64_t)be64toh(*(int64_t *)ptr);
}
...
read_again:
		...
		if (sizeof(time_t) == 8 && trans_width == 4 && tzhead.tzh_version[0] != '\0') {
			...
			/* Position the stream before the second header.  */
				...
				if (fseek(f, (long)to_skip, SEEK_CUR) != 0)
					return -EINVAL;
		...
		transitions = calloc((total_size + tzspec_len + 1), 1);
		if (transitions == NULL)
			return -EINVAL;
		...
		/* Assume that DST is changing twice a year and guess initial
		   ...
		   = 15778476 seconds.  */
		i = (size_t)((transitions[num_transitions - 1] - date) / 15778476);
...
found:
		...
		if (delta_next)
			*delta_next = (int)((types[type_idxs[i]].offset - types[type_idxs[i-1]].offset) / 60);
		if (zone_next)
